...
Pour diminuer la taille de la base de données, il est conseillé de mettre dans un cron journalier la commande de nettoyage suivante :
...
Le 1er de chaque mois, il faut exécuter le script suivant (celui-ci ne peut s'exécuter que le 1 du mois).
